...
seconds_left <- var_seconds %% 60

min_sen <- if (var_minutes == 1) "minute" else "minutes"
sec_sen <- if (var_seconds == 1) "second" else "seconds"

cat(var_seconds, "is", var_minutes, min_sen, "and", seconds_left,sec_sen)
...
if (num1 >= num2 && num1 >= num3) {
  cat(num1, "is the largest number")
} else if (num2 >= num1 && num2 >= num3){
  cat(num2, "is the largest number")
} else {
  cat(num3, "is the largest number")
}
...
cat("the final factorial number is:",(num))
...
for (i in 1:y) {
 if (i^2 > y) {
  cat(i, "squared is", i^2, "and its the first number whose square excedds", y, "\n")
    break
  }
}
```
